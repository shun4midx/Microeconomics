# Microeconomics
Shun's Notes for Principles of Microeconomics with Recitation

[Here](Notes/notes.pdf) is the most up-to-date PDF of my notes, and [here](Notes/flashcards.pdf) is the most up-to-date PDF of what I deem are terminology and other key points that I make into flashcards.

For convenience, I also upload PDFs of notes and flashcards in individual chapters, as can be found by clicking on the respective PDF and downloading [here](Notes/indiv_chapters).

I don't really like using actual "flashcards" and usually just handwrite things in a color-coded way so it's easier for me to memorize stuff. In [here](Written_Flashcards), I upload my handwritten versions of what flashcards in each individual chapter include. Theoretically, they should contain the same information as those I typed up but be more colorful, but may be reformated and combined with some information in the notes sometimes in ways that make it easier to memorize. To study these chapters, please remember to first understand all of the notes of each chapter before studying the flashcards.

## Color Schemes
It is not a secret I like the [Pink/Purple/Blue color scheme](https://github.com/shun4midx/Pink-Purple-Blue-VSCode-Theme). As a result, my more recent LaTeX files for notes all follow this aesthetic, typically with **blue boxes for headings**, **purple boxes for subheadings**, and **pink boxes for important concepts**. I also typically use yellow highlighter and very rarely green if there is too much yellow. 

If for some reason you do not like this color scheme, all boxes and highlights are drawn via commands in `shun4colors.tex`, so feel free to clone this repo and modify the colors in the commands yourself for personal use. On the other hand, if for some reason you like the color scheme I use, feel free to use my usual import files for personal use, where more detail can be found [here](https://github.com/shun4midx/LaTeX-Template). I would appreciate it if you credit me for my templates.
